...
# - Se hace segmentación (el tamaño de la ventana es igual al tamaño de la ventana de corazon)
##########################################################################################################
...
k = 2 # Numero de periodos que componen una ventana
nbConv = int(k * 60/brate_min * fs) # Conversiones totales de una ventana
nhConv = int(k * 60/hrate_min * fs)
nConv = nhConv
r = nbConv/nhConv
endSampling = 0
vueltas = 0
...
def my_callback(channel):
    global RDY, ads, chan, conv, endSampling, nConv, vueltas, numSampling
    if (conv % nhConv) == 0:
        print(conv)
#     nb.append(int(chan.value))
#     nh.append(nb[-1])
    i = conv * numSampling
    nb.append(yn[i])
    nh.append(nb[-1])
...
while num <= nSlot:
    ...
    if numSampling % 5 == 0: # Si ya se han muestreado 5 ventanas, se transfieren las muestras de resp.
        mb = nb
        nb = []
